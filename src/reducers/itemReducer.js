...
import {GET_ITEMS, ADD_ITEMS, DELETE_ITEM, TOGLLE_NAV, CLOSE_NAVBAR,
     BANER_CLOSE, DECREASE_ITEM, INCREASE_ITEM, TOTAL_ITEMS, DETAILS} from '../actions/types';
...
//   add items to cart
  case ADD_ITEMS:
   ...
   if(!check){
    //bringing only the item that match the id
...
// in order to set isincart to false we create an action
  case BANER_CLOSE :
...
case DELETE_ITEM:
 ...
 // increasing item inside the cart
 case INCREASE_ITEM :
...
// decreasing item inside the cart
case DECREASE_ITEM :

//   let tempItem = state.cart.filter(item=>item._id===action.payload);
